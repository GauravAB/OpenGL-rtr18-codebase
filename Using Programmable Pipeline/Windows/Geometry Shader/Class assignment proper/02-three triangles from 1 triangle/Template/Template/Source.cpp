...
void initialize(void)
{
	...
	glShaderSource(gGeometryShaderObject, 1, (GLchar**)&geometryShaderSourceCode, NULL);
	glCompileShader(gGeometryShaderObject);
	iCompileStatus = 0;
	iInfoLogLength = 0;
	szInfoLog = NULL;

	glGetShaderiv(gGeometryShaderObject, GL_COMPILE_STATUS, &iCompileStatus);
	if (iCompileStatus == GL_FALSE)
	{
		...
		if (iInfoLogLength > 0)
		{
			...
			fprintf(gpFile, "GeometryShader compile error Log %s", szInfoLog);
			free(szInfoLog);
			uninitialize();
			exit(0);
		}
