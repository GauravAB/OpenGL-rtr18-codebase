...
void initialize(void)
{
	...
	parse->getModelVertexData(&sphereVertices, &sphereNormals, &sphereTextures, &sphereElements);

	numElements = parse->getNumElements();
	numVertices = parse->getNumVertices();
...

{
	...
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, gVbo_elements);
	glDrawElements(GL_TRIANGLES, numElements, GL_INT, 0);
	glBindVertexArray(0);
