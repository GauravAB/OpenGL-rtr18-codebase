...
void initialize(void)
{
	...
	const GLchar *fragmentShaderSourceCode =
		...
		"vec3 specular = u_Ls * u_Kd * pow(max(dot(reflection_vector,normalized_viewer_vector),0.0),u_material_shininess);"\
	...
	float* sphere_vertices = (float*)calloc(modelPositionCoords.size(),sizeof(float));
	float* sphere_normals = (float*)calloc(modelNormalCoords.size(), sizeof(float));
	float* sphere_textures = (float*)calloc(modelTexCoords.size(), sizeof(float));
	short* sphere_elements = (short*)calloc(modelElements.size(), sizeof(short));

	for (int i = 0; i != modelPositionCoords.size(); i++)
	{
		sphere_vertices[i] = modelPositionCoords[i];
	}

	for (int i = 0; i != modelNormalCoords.size(); i++)
	{
		sphere_normals[i] = modelNormalCoords[i];
	}

	for (int i = 0; i != modelTexCoords.size(); i++)
	{
		sphere_textures[i] = modelTexCoords[i];
	}

	for (int i = 0; i != modelElements.size(); i++)
	{
		sphere_elements[i] = modelElements[i];
	}
	...
	*/
	...
	gNumVertices = getNumVertices();
	gNumElements = getNumElements();
