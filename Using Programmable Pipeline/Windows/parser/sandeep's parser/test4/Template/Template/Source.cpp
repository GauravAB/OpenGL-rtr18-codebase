...
#pragma comment(lib,"ModelParser.lib")
...
IModelLoader *ml = getModelLoader();
...
void initialize(void)
{
	...
	float *sphereVertices, *sphereNormals, *sphereTextures;
	int numSphereVertices,  numSphereTextures, numSphereNormals;


	
	ml->LoadMeshData("sphere.obj", &sphereVertices, &sphereTextures, &sphereNormals, &sphereElements, &numSphereVertices, &numSphereTextures, &numSphereNormals, &numElements);
	...
	glVertexAttribPointer(GAB_ATTRIBUTE_TEXTURE0, 3, GL_FLOAT, GL_FALSE, 0, NULL);
...
void uninitialize(void)
{
	...
	if (gVbo_positions)
	{
		glDeleteBuffers(1, &gVbo_positions);
		gVbo_positions = 0;
	}
	...
	if (gVbo_textures)
	{
		glDeleteBuffers(1, &gVbo_textures);
		gVbo_textures = 0;
	}
