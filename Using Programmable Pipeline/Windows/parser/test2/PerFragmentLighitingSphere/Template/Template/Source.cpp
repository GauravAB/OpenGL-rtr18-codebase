...
void initialize(void)
{
	...
	float* sphere_vertices, *sphere_normals, *sphere_textures;
	int *sphere_elements;
	
	pObj->parseObj("Sphere.obj");
	pObj->getModelVertexData(&sphere_vertices, &sphere_normals, &sphere_textures, &sphere_elements);

	//data array
	gNumVertices = pObj->getNumVertices();
	gNumElements = pObj->getNumElements();

	if (sphere_vertices == NULL || sphere_normals == NULL || sphere_textures == NULL || sphere_elements == NULL)
	{
		MessageBox(NULL, TEXT("VertexAttribData retrieval failed"), L"Error", MB_OK);
		exit(0);
	}
	else
	{
		fprintf(gpLogFile, "putting data");
		for (int i = 0; i < gNumVertices; i++)
		{
			fprintf(gpLogFile, "Vertex[%d] = %d", i, sphere_vertices[i]);
			fclose(gpLogFile);
		}	
		exit(0);
	}
