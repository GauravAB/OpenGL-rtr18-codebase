...
void Parser::getModelVertexData(float** vert, float** norm, float** tex, int **elements)
{
	...
	/*
	modelElements = (int*)malloc(sizeof(int)*maxElements);
	modelPositions = (float*)malloc(sizeof(float)*Vertices*3*elements_size*point_coordinates);
	modelNormals = (float*)malloc(sizeof(float)*Normals*3*elements_size*normal_coordinates);
	modelTextures = (float*)malloc(sizeof(float)*Textures*2*texture_coordinates*elements_size);
	*/
...
void Parser::addTriangle(float single_vertexPos[3][3],float single_texture[3][2] , float single_normal[3][3])
{
	...
	else
	{
		...
		modelPositions = (float*)realloc(modelPositions, sizeof(float)*(numVertices + 3) * 3 * 3);
		modelNormals = (float*)realloc(modelNormals, sizeof(float)*(numVertices + 3) * 3 * 3);
		modelTextures = (float*)realloc(modelTextures, sizeof(float)*(numVertices + 3) * 3 * 2);
	}
