...
GLuint gSpecularMaterialUniform;
GLuint gShininess;
GLuint gPatchMatrixUniform;
GLuint gTransposedPatchMatrixUniform;
...
void initialize(void)
{
	...
	gSpecularMaterialUniform = glGetUniformLocation(gShaderProgramObject, "specularMaterial");
	gShininess = glGetUniformLocation(gShaderProgramObject, "shininess");
	gPatchMatrixUniform = glGetUniformLocation(gShaderProgramObject, "B");
	gTransposedPatchMatrixUniform = glGetUniformLocation(gShaderProgramObject, "BT");




	//data array
	gNumVertices = sizeof(PatchData) / (sizeof(float) * 3);
	...
	//copy the data
	glBufferData(GL_ARRAY_BUFFER, sizeof(float)*3*gNumVertices,PatchData, GL_STATIC_DRAW);
...

{
	...
	vec3 eye = vec3(0.0,50.0,0.0);
	vec3 center = vec3(0.0, 0.0, 0.0);
	vec3 up = vec3(0.0, 0.0, 1.0);

	mat4 bezier = mat4(vec4(-1.0, 3.0, -3.0, 1.0),
		vec4(3.0, -6.0, 3.0, 0.0),
		vec4(-3.0, 3.0, 0.0, 0.0),
		vec4(1.0, 0.0, 0.0, 0.0));


	modelViewMatrix = modelViewMatrix * lookat(eye,center,up);
	modelViewMatrix *= translate(0.0f, 0.0f, -10.0f);
	modelViewMatrix *= rotate(timerUniform, 0.0f, 0.0f, 1.0f);
	...
	glUniformMatrix4fv(gPatchMatrixUniform,1,GL_FALSE,bezier);
	glUniformMatrix4fv(gTransposedPatchMatrixUniform, 1, GL_TRUE, bezier);

	glBindVertexArray(gVao);

	glPatchParameteri(GL_PATCH_VERTICES, 16);
	glDrawArrays(GL_PATCHES, 0, gNumVertices);
