...
void initialize(void)
{
	...
	const GLubyte *szOGLVersion;
	const GLubyte *szGLSLVersion;
	const GLubyte *vendor;
	
	//glGetIntegerv(GL_NUM_EXTENSIONS, &extensions_num);
	szOGLVersion = glGetString(GL_SHADING_LANGUAGE_VERSION);
	szGLSLVersion = glGetString(GL_VERSION);
	vendor = glGetString(GL_VENDOR);

	fprintf(gpFile, "opengl vendor: %s \n", vendor);
	fprintf(gpFile, "%s ", szOGLVersion);
