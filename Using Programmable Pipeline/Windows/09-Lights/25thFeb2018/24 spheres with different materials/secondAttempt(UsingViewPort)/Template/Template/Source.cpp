...

{
	...
	for (int i = 0; i != 6; i++)
	{
		for (int j = 0; j != 4; j++)
		{
			...
			glBindVertexArray(gVao_sphere);
			glViewport(i * gWidth/7.5, j * gHeight/4.2 , gWidth/2.95, gHeight/3);
			modelMatrix = mat4::identity();
			modelMatrix = modelMatrix * translate(0.0f, 0.0f, -3.0f);
			glUniform3fv(Ka_uniform, 1, material_ambient[i * 4 + j]);
			glUniform3fv(kd_uniform, 1, material_diffuse[i * 4 + j]);
			glUniform3fv(Ks_uniform, 1, material_specular[i * 4 + j]);
			glUniform1f(material_shininess_uniform, material_shininess[i * 4 + j]);
