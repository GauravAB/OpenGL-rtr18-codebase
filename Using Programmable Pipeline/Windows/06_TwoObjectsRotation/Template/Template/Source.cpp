...
GLuint gVaoTriangle;
GLuint gVaoSquare;
GLuint gVboSquarePos;
GLuint gVboTrianglePos;
...
void initialize(void)
{
	...
	glGenBuffers(1, &gVboTrianglePos);
	glBindBuffer(GL_ARRAY_BUFFER, gVboTrianglePos);
	...
	glGenBuffers(1, &gVboTriangleColor);
	glBindBuffer(GL_ARRAY_BUFFER, gVboTriangleColor);
	glBufferData(GL_ARRAY_BUFFER, sizeof(float) * 9, triangleColor, GL_STATIC_DRAW);
	...
	glGenBuffers(1, &gVboSquarePos);
	glBindBuffer(GL_ARRAY_BUFFER, gVboSquarePos);
	glBufferData(GL_ARRAY_BUFFER, sizeof(float) * 12, squareVertices, GL_STATIC_DRAW);
	glVertexAttribPointer(GAB_ATTRIBUTE_VERTEX, 3, GL_FLOAT, GL_FALSE, 0, NULL);
	glEnableVertexAttribArray(GAB_ATTRIBUTE_VERTEX);
	glBindBuffer(GL_ARRAY_BUFFER, 0);

	glVertexAttrib3f(GAB_ATTRIBUTE_COLOR, 1.0, 1.0f, 0.0f);
	//glEnableVertexAttribArray(GAB_ATTRIBUTE_COLOR);
...

{
	...
	modelViewMatrix = modelViewMatrix * translate(-1.4f, 0.0f, -6.0f);
	modelViewMatrix = modelViewMatrix * rotate(angle_y,0.0f, 1.0f, 0.0f);
	...
	modelViewMatrix = modelViewMatrix * translate(0.8f, 0.0f, -3.0f);
	modelViewMatrix = modelViewMatrix * rotate(angle_y, 0.0f, 1.0f, 0.0f);
...
void uninitialize(void)
{
	...
	if (gVboTrianglePos)
	{
		glDeleteBuffers(1,&gVboTrianglePos);
		gVboTrianglePos = 0;
	}
	...
	if (gVboSquarePos)
	{
		glDeleteBuffers(1, &gVboSquarePos);
		gVboSquarePos = 0;
	}
