...
{
	GLfloat angle = 0.0f;
	GLint circle_points = 1000 , i;
	GLfloat sideA, sideB, sideC , S , radius ,xoff, yoff;
	...
	sideA = sqrt(pow((-1.0 - 0.0), 2) + pow((-1.0 - 1.0), 2));
	sideB = sqrt(pow((1.0 - 0.0), 2) + pow((-1.0 - 1.0), 2));
	sideC = sqrt(pow((-1.0 - 1.0), 2) + pow((-1.0 - (-1.0)), 2));
	
	S = (sideA + sideB + sideC) / 2;
	radius = sqrt( ((S - sideA)*(S - sideB)*(S - sideC))/S);
	xoff = (0.0 + (-1.0) + 1.0) / 3;
	yoff = (1.0 + (-1.0) + 1.0) / 3;
	...
	for (i = 0; i < circle_points; i++)
	{
		...
		glVertex2f(radius*cos(angle)+xoff, radius*sin(angle)-(yoff+0.05));
	}
	glEnd();
