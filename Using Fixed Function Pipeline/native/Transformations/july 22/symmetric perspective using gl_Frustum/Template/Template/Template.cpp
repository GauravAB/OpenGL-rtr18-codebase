...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		case 0x43:
			...
			break;
		case 0x54:
			if (gbDrawTriangle == false)
			{
				gbDrawTriangle = true;
			}
			else
			{
				gbDrawTriangle = false;
			}
			break;
		case VK_ESCAPE:
...
void resize(int width, int height)
{
	...
	//gluPerspective(0.0f,(GLfloat)width/(GLfloat)height,2.1f,100.0f);	//comment about the angle 
	glFrustum(-50.0f, 50.0f, -50.0f, 50.0f, 50.0, 100.0f);
	/*if (width <= height)
	{
	...
	{
		glFrustum(-1.0f,1.0f,-1.0f*(GLfloat)width / (GLfloat)height,1.0f* (GLfloat)width / (GLfloat)height,-1.0f, 1.0f);
	}*/
}
