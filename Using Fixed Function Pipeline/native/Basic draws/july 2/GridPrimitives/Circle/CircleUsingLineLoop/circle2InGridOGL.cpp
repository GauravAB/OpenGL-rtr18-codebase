...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR nCmdLing, int nCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("my circle using line loop"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, wPos - (WIN_WIDTH / 2), hPos - (WIN_HEIGHT / 2), WIN_WIDTH, WIN_HEIGHT, 0, 0, hInstance, 0);
...
void display(void)
{
	...
	void draw_grid(GLfloat width);	
	void draw_circle(GLsizei size , GLfloat radius);
...
void draw_circle(GLsizei size, GLfloat radius)
{
	...
	glBegin(GL_LINE_LOOP);
		...
		{
			//smart way to increment angle with respect to the counter var , when counter reaches max we acheive 2*PI complete 
			angle = 2 * PI* i/circle_points;
			//plot cosine and sine
			//note that the range of cosine and sine [-1<=Fn<=1]
			//using polar equation of circle i.e x = rcos(theta) , y = rsine(theta);
